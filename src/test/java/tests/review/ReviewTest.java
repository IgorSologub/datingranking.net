...
public class ReviewTest extends BaseTest {
    ...
    @Test(invocationCount = 2, successPercentage = 99)
    public void popularPagesButtonTest () {
        basePage.closePopup();
        int goodTitlesCount = 0;
        for (int i = 1; i <= NUMBER_OF_POPULAR_PAGES_REVIEW_PAGE; i++) {
            basePage.waitOneSeconds();
            reviewPage.popularPagesButtonClick(i);
            basePage.waitOneSeconds();
            if (basePage.checkTitles(REVIEW_TITLES)) { goodTitlesCount++; }
            else { basePage.outputWrongTitle(); }
            basePage.back(REVIEW_PAGE_URL);
        }
        assertEquals(NUMBER_OF_POPULAR_PAGES_REVIEW_PAGE, goodTitlesCount);
    }
